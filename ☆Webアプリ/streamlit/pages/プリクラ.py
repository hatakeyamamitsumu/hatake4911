...
def main():
    ...
    # 一番手前の画像をアップロード
    front_image = st.file_uploader("一番手前に重ねる画像をアップロードしてください", type=["jpg", "jpeg", "png"])
    ...
    if front_image is not None:
        ...
        # 背景を除去
        front_image_obj = remove_background(front_image_obj)
...
def remove_background(image):
    ...
    alpha = image.split()[-1]
    # アルファマスクを使って背景を除去
    image = ImageOps.invert(ImageOps.grayscale(alpha))
    return image
