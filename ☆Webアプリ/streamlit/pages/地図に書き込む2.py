...
if app_selection == "地図にピンを立て、コメントをつけて保存する":
    ...
    # 緯度経度の入力欄
    with st.sidebar.expander("緯度経度入力"):
        # 緯度の入力
        latitude_input = st.number_input("緯度を入力してください", step=0.000001, format="%.6f")
        # 経度の入力
        longitude_input = st.number_input("経度を入力してください", step=0.000001, format="%.6f")
