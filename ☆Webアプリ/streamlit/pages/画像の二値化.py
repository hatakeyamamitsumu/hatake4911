...
def colorize_binary_image(original_color_image, binary_image, color=(0, 0, 255)):
    # 2値化画像の黒い領域に対応する位置を取得
    black_pixels = binary_image == 0

    # 元のカラー画像のコピーを作成
    colored_image = original_color_image.copy()

    # 2値化された領域に対応するピクセルの色を変更
    colored_image[black_pixels] = color
...
def main():
    ...
    if radio=="Threshold" and uploaded_image is not None:
        col2.image(th1)
        colored_binary_image = colorize_binary_image(img, th1, color=(0, 0, 255))
        cv2.imwrite('./data/image.png', colored_binary_image)
    elif radio=="Adaptive threshold mean" and uploaded_image is not None:
        col2.image(th2)
        colored_binary_image = colorize_binary_image(img, th2, color=(0, 0, 255))
        cv2.imwrite('./data/image.png', colored_binary_image)
    elif radio=="Adaptive threshold Gaussian" and uploaded_image is not None:
        col2.image(th3)
        colored_binary_image = colorize_binary_image(img, th3, color=(0, 0, 255))
        cv2.imwrite('./data/image.png', colored_binary_image)
    elif radio=="Otsu' thresholding" and uploaded_image is not None:
        col2.image(th4)
        colored_binary_image = colorize_binary_image(img, th4, color=(0, 0, 255))
        cv2.imwrite('./data/image.png', colored_binary_image)
    elif radio=="Otsu's thresholding + Gaussian fileter" and uploaded_image is not None:
        col2.image(th5)
        colored_binary_image = colorize_binary_image(img, th5, color=(0, 0, 255))
        cv2.imwrite('./data/image.png', colored_binary_image)
