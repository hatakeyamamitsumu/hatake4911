...
if app_selection == "地図にピンを立て、コメントをつけて保存する":
    ...
    latitude_input = st.sidebar.number_input("南北に１００ｍ移動　(緯度コピペ欄)",step=0.001,format="%.4f",key="latitude")
    longitude_input = st.sidebar.number_input("東西に１００ｍ移動　(経度コピペ欄)",step=0.001,format="%.4f",key="longitude")
