...
def plot_3d_scatter(data, x_col, y_col, z_col, index_col, show_regression=False):
  fig = px.scatter_3d(data, x=x_col, y=y_col, z=z_col, text=index_col)
  if show_regression:
    if len(data) >= 10:
      fig.add_trace(px.scatter_3d(data, x=x_col, y=y_col, z=z_col, trend='ols'))
    else:
      st.warning('データ数が少ないため、回帰直線/面を表示できません。データ数を10以上にしてください。')
  st.plotly_chart(fig)

def plot_2d_scatter(data, x_col, y_col, index_col, show_regression=False):
  fig = px.scatter(data, x=x_col, y=y_col, text=index_col)
  if show_regression:
    if len(data) >= 10:
      fig.add_trace(px.scatter(data, x=x_col, y=y_col, trend='ols'))
    else:
      st.warning('データ数が少ないため、回帰直線/面を表示できません。データ数を10以上にしてください。')
  st.plotly_chart(fig)

def plot_scatter(data, selected_columns, index_col, dimensions, show_regression):
  if dimensions == 2:
    plot_2d_scatter(data, selected_columns[0], selected_columns[1], index_col, show_regression)
  elif dimensions == 3:
    plot_3d_scatter(data, selected_columns[0], selected_columns[1], selected_columns[2], index_col, show_regression)
...
        show_regression_utf8 = st.checkbox('回帰直線/面を表示')
        st.write(f'### {dimensions_utf8}次元散布図 (UTF-8):')
        plot_scatter(data_utf8, selected_columns_utf8, index_col_utf8, dimensions_utf8, show_regression_utf8)
      else:
...
        index_col_shiftjis = st.selectbox('見出し列を選んでください（見出し数が1
