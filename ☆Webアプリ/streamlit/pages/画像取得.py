...
# ダウンロード用のHTMLリンクを生成するヘルパー関数
def get_binary_file_downloader_html(bin_file, file_label='動画ファイル'):
    with open(bin_file, 'rb') as f:
        data = f.read()
    bin_str = base64.b64encode(data).decode()
    href = f'<a href="data:application/octet-stream;base64,{bin_str}" download="{file_label}">ダウンロード</a>'
    return href
...
def main():
    ...
    if videos:
        ...
        if st.button('動画をダウンロード'):
            st.markdown(get_binary_file_downloader_html(downloaded_video, selected_video_name), unsafe_allow_html=True)
