...
def main():
    ...
    if videos:
        ...
        if st.button('動画をダウンロード'):
            download_video_to_disk(downloaded_video, selected_video_name)

# ダウンロード用の関数
def download_video_to_disk(bin_file, file_label='動画ファイル'):
    with open(f"{file_label}.mp4", "wb") as f:
        f.write(bin_file.getvalue())
    st.success("動画をダウンロードしました。")
