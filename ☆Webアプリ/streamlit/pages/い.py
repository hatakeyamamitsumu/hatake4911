...
# Seleniumを使用してGoogle Imagesから画像URLを取得
def fetch_image_urls_google(query:str, max_links_to_fetch:int, wd):
...
# BeautifulSoupを使用してウェブページから画像URLを取得
def fetch_image_urls_web(query:str, max_links_to_fetch:int):
...
if st.button("Googleからクローリング＆表示"):
    if keyword:
        st.write(f"{keyword} に関連する画像をクローリングしています...")
        image_urls_google = fetch_image_urls_google(query=keyword, max_links_to_fetch=max_images, wd=webdriver.Chrome(executable_path="./chromedriver"))
