...
def filter_and_download(text, selected_conditions, condition_type):
    ...
    if filtered_lines:
        ...
        if result_lines:
            ...
            # 結果のデータフレーム表示
            result_df = pd.DataFrame({'抽出された文': result_lines})
            st.write("### 抽出された文のデータフレーム")
            st.dataframe(result_df)

            # 抽出した文の長さの統計情報
            lengths = [len(line) for line in result_lines]
            st.write("### 抽出された文の長さの統計情報")
            st.write(f"最小長さ: {min(lengths)}")
            st.write(f"最大長さ: {max(lengths)}")
            st.write(f"平均長さ: {sum(lengths) / len(lengths)}")
        else:
